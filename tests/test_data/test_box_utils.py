...
def test_clean_boxes():
    # Valid boxes
    assert clean_boxes([[0.1, 0.2, 0.3, 0.4], [0.1, 0.2, 0.3, 0.4]]) == [[0.1, 0.2, 0.3, 0.4], [0.1, 0.2, 0.3, 0.4]]
    
    # Degenerate boxes (x2<=x1 or y2<=y1)
    assert clean_boxes([[0.3, 0.4, 0.1, 0.2], [0.1, 0.2, 0.1, 0.4]]) == []
    
    # Non-numeric
    assert clean_boxes([["a", "b", "c", "d"]]) == []
    
    # Wrong length
    assert clean_boxes([[0.1, 0.2, 0.3]]) == []
    
    # Out of bounds
    assert clean_boxes([[1.5, 0.2, 2.0, 0.4]]) == []
...
def test_is_valid_box():
    # Valid
    assert is_valid_box([0.1, 0.2, 0.3, 0.4]) is True
    
    # None
    assert is_valid_box(None) is False
    
    # Wrong length
    assert is_valid_box([0.1, 0.2, 0.3]) is False
    
    # Non-numeric
    assert is_valid_box([0.1, "0.2", 0.3, 0.4]) is False
    
    # Zero-area (or very small)
    assert is_valid_box([0.1, 0.2, 0.1, 0.4]) is False
