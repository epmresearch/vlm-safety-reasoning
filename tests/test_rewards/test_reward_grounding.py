...
class TestRewardGrounding:
    def test_true_negative_all_classes_returns_one(self):
        """No objects predicted, no objects in GT → 1.0 (correct TN for all classes)."""
    ...
    def test_perfect_true_positive_excavator(self):
        ...
        score = compute_reward(completion, gt)
        # Perfect IoU → 0.5 (presence) + 0.5*1.0 (IoU) = 1.0 for excavator
        # Other 2 classes: TN → 1.0 each
    ...
    def test_false_positive_single_class(self):
        ...
        score = compute_reward(completion, gt)
        # excavator: FP → 0.0; rebar: TN → 1.0; hat: TN → 1.0
        assert score == pytest.approx(2.0 / 3.0, abs=0.02)
    ...
    def test_false_negative_single_class(self):
        ...
        gt = _gt(excavator_01=[[0.1, 0.05, 0.8, 0.7]])
        score = compute_reward(completion, gt)
        # excavator: FN → 0.0; others: TN → 1.0
        assert score == pytest.approx(2.0 / 3.0, abs=0.02)
    ...
    def test_low_iou_tp(self):
        ...
        score = compute_reward(completion, gt)
        # IoU < 1.0, score for excavator = 0.5 + 0.5*iou, total avg < 1.0
    ...
    def test_scale_conversion_correctness(self):
        ...
        score = compute_reward(completion, gt)
        # Perfect overlap → excavator score = 1.0
        assert score == pytest.approx(1.0, abs=0.02)
    ...
    def test_multiple_classes_independently_scored(self):
        ...
        score = compute_reward(completion, gt)
        # excavator: TP perfect → 1.0, rebar: FN → 0.0, hat: TN → 1.0
        assert score == pytest.approx(2.0 / 3.0, abs=0.02)