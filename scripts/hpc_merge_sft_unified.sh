#!/usr/bin/env bash
#SBATCH --job-name=vlm-merge-sft-unified
...
#SBATCH --output=/home/%u/vlm-finetuning-project1/logs/merge_sft_unified_%j.out
#SBATCH --error=/home/%u/vlm-finetuning-project1/logs/merge_sft_unified_%j.err
...
if [ -z "$TIER" ] || [ -z "$SFT_VARIANT" ] || [ -z "$MERGED_VARIANT_NAME" ]; then
    echo "Error: Arguments missing (Usage: hpc_merge_sft_unified.sh <tier> <sft_variant> <merged_variant_name>)"
    exit 1
fi
...
echo "======================================================================"
echo "Merging SFT adapter for tier ${TIER} (Unified Task)"
