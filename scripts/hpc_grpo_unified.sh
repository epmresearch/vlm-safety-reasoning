#!/usr/bin/env bash
#SBATCH --job-name=vlm-grpo-unified
...
#SBATCH --output=/home/%u/vlm-finetuning-project1/logs/grpo_unified_%j.out
#SBATCH --error=/home/%u/vlm-finetuning-project1/logs/grpo_unified_%j.err
...
if [ -z "$TIER" ] || [ -z "$GRPO_VARIANT" ] || [ -z "$MERGED_VARIANT_NAME" ]; then
    echo "Error: Arguments missing (Usage: hpc_grpo_unified.sh <tier> <grpo_variant> <merged_variant_name>)"
    exit 1
fi
...
echo "======================================================================"
echo "[STEP 1/4] Running GRPO on ${TIER} model (Unified Task)"
echo "======================================================================"
...
# (experiments/run_grpo.py itself also refuses to start without --base_model_override
# or an explicit --allow_unmerged_reference ablation flag, so this check just fails
# fast with a clearer message before that.)
if [ -d "${MERGED_BASE}" ] && [ -f "${MERGED_BASE}/config.json" ]; then
    ...
    echo "Run hpc_merge_sft_unified.sh first, or check that the merge job completed successfully."
