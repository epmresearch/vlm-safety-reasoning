...
**Note:** `output_format` also appears as a key in every `configs/tasks/*.yaml` (e.g. `"fenced_minimized_json"`).
It is decorative — nothing reads it; the live source of truth is `core/tasks.py::TaskSpec.output_format`
(`"fenced_json"`/`"plain_text"`), a different vocabulary. Don't wire the YAML key in without reconciling the two.
