...
#   - Resolution-bucketed batching (data/samplers.py) to prevent padding-blowup OOM
#   - image_min_pixels/max_pixels capping to neutralize 4K/14MP outliers
#   - auto_find_batch_size as a first-line OOM guard
#   - wandb run id persisted at train START (not end) -> resume actually resumes
#   - SaveBestModelCallback: best adapter saved to Drive on every eval improvement,
#     independent of checkpoint rotation and independent of the run finishing
#   - ManifestUpdateCallback: crash-safe training_state.json, updated continuously
#   - EarlyStoppingCallback (optional)
#   - GPU memory logging to W&B
# """
# import json
# from pathlib import Path
# from typing import Any, Dict, List, Optional
...
# def _build_bucketed_trainer_class(sampler_batch_size: int, resolutions: List[float], cfg: Dict[str, Any]):
#     """Returns an SFTTrainer subclass whose train dataloader uses ResolutionBucketSampler."""
#     from trl import SFTTrainer
#     from torch.utils.data import DataLoader
#     from data.samplers import ResolutionBucketSampler
...
#     Args:
#         tier: Model tier ("2b", "4b", "8b").
#         variant: Checkpoint variant name (e.g., "unified-sft-v1"). Use a NEW
#             variant per ablation (e.g. "unified-sft-lora_r8") so runs never collide.
#         train_dataset / val_dataset: Unsloth conversation dicts.
#         train_resolutions: Per-sample pixel counts, same order/length as train_dataset.
#             Required if `use_resolution_bucketing` is enabled in sft_cfg.
#         sft_cfg: SFT config dict. If None, loads configs/sft.yaml.
#         resume: Whether to resume from the latest checkpoint + wandb run.
#         run_name: W&B run name. Defaults to "{short_name}-{variant}".
...
#         report_to=["wandb"],
#         run_name=run_name,
#         auto_find_batch_size=sft_cfg.get("auto_find_batch_size", True),
#         remove_unused_columns=False,
#         dataset_kwargs={"skip_prepare_dataset": True},
#     )
...
#         "wandb_run_id": run.id if run else None,
#     }
#     # Persist immediately, before a single training step runs.
#     ensure_dir(checkpoint_dir)
#     with open(checkpoint_dir / "training_state.json", "w") as f:
#         json.dump({**static_manifest_fields, "status": "starting"}, f, indent=2)
...
#     # --- Trainer (bucketed or plain) ---
#     use_bucketing = sft_cfg.get("use_resolution_bucketing", True) and train_resolutions is not None
#     trainer_kwargs = {
#         "model": model,
#         "tokenizer": tokenizer,
#         "train_dataset": train_dataset,
...
#     if use_bucketing:
#         logger.info("Resolution bucketing ENABLED for train dataloader.")
#         TrainerClass = _build_bucketed_trainer_class(
#             sampler_batch_size=batch_cfg["per_device_train_batch_size"],
#             resolutions=train_resolutions,
#             cfg=sft_cfg,
#         )
#     else:
#         logger.info("Resolution bucketing DISABLED (falling back to default shuffling).")
#         from trl import SFTTrainer as TrainerClass
...
#         raise
#     finally:
#         # Always attempt to persist whatever we have, even on interrupt/crash
#         final_dir = checkpoint_dir / "final"
#         try:
#             ensure_dir(final_dir)
#             model.save_pretrained(str(final_dir))
...
#     # Prefer the crash-safe `best/` dir as the canonical output if it exists
#     return str(best_dir) if best_dir.exists() else str(checkpoint_dir / "final")